---
title: Submerged 2汉化下载
icon: 
description: |
  **Submerged 2** 将你带入一个绝美的、完全自定义的水下世界。这里充满了全新的挑战、独特的地形和重重惊喜。
updateDate: 2026年02月21日
status:
  text: 维护中
  type: success
compatibility:
  loader: neoforge
  minecraft: '1.21.1'
  pack: 'B9.0'
links:
  - id: curseforge
    text: 下载i18n模组
    link: https://www.curseforge.com/api/v1/mods/297404/files/7173159/download
  - id: github
    text: GitHub仓库
    link: https://github.com/VM-Chinese-translate-group/Submerged-2-Chinese
  - id: paratranz
    text: Paratranz
    link: https://paratranz.cn/projects/17732
---

<script setup>
import DownloadLayout from "@/components/DownloadPage/DownloadLayout.vue"
</script>

<DownloadLayout :meta="frontmatter">

**Submerged 2** 将你带入一个绝美的、完全自定义的水下世界。这里充满了全新的挑战、独特的地形和重重惊喜。
你的旅程始于深海中一个密封的玻璃球体内——你需要通过战斗、建造和创新，在这个试图排斥你的世界中开辟出一条繁荣之路。

![图片](https://media.forgecdn.net/attachments/1397/106/bae-png.png)

<DownloadLinks :methods="[
  { id: 'quark-lanzou', text: '下载汉化', icon: '/imgs/logo/logo_64.png', lanzouLink: '/doing', quarkLink: '/doing' },
  { id: 'curseforge', text: 'i18n自动汉化更新模组', icon: '/imgs/svg/curseforge.svg', link: 'https://www.curseforge.com/api/v1/mods/297404/files/7173159/download' },
  { id: 'github', text: 'GitHub仓库', icon: '/imgs/svg/github.svg', link: 'https://github.com/VM-Chinese-translate-group/Submerged-2-Chinese' },
  { id: 'lazy', text: '懒汉下载', icon: '/imgs/lazydl.png', link: '/doing' }
]" />

## 更多信息

| 整合包下载地址                                                    | 加载器   | 支持整合包版本 | 汉化状态 |
| :---------------------------------------------------------------- | :------- | :------------- | :------- |
| [链接](https://www.curseforge.com/minecraft/modpacks/submerged-2) | Neoforge | mc1.21.1 B9.0  | 翻译中   |

译者：[查看贡献人员排行榜](https://paratranz.cn/projects/17732/leaderboard)

## 特色

凭借**机械动力**、**农夫乐事**、**新生魔艺**、**力量工艺**、**工业先锋** 等强力模组，
以及 **Colors**、**Casting**、**Routers**、**Invisible Lights** 等备受社区欢迎的辅助模组，
Submerged 2 赋予了你自由定义玩法的高度。

倾向于复杂的机械结构？那就选择机械动力。渴望魔法？试试新生魔艺或力量工艺。热爱技术进阶？投身于**气动工艺**吧。

**没有任何路径是锁定的——以你喜欢的方式去游玩！**

- **空间球钥匙**：让水下建筑成为梦想——瞬间为你扩展新的基地区域。
- **Flares**：空投装满物资的木桶，助你度过生存难关。
- **有毒空气与水源**：时刻保持警惕，注意环境威胁。
- **Loot Pinatas**：击碎即可获得令人满足的宝藏，内含丰厚的自定义章节奖励。
- **海量改进**、隐藏秘密和独特的水下反转等待着你去发现。

## 整合包下载

你可以先在 [CurseForge](https://www.curseforge.com/minecraft/modpacks/submerged-2) 或启动器内下载整合包，然后下载汉化补丁，最后将补丁解压并覆盖相应文件，即可完成汉化。

## 支持版本

支持整合包最新版本。

<DocSupport />

</DownloadLayout>
